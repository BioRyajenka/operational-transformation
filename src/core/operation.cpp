...
void merge_chains(
    ...
    if (a.get_root()->value.id < b.get_root()->value.id) {
        ...
        left->insertions[source_node_id] = chain(b.get_root());
...
//        const bool &copy_right_part
    ...
    const auto left = std::make_shared<operation>();
    ...
    // === process deletions ===
    process_deletions(rhs.deletions, deletions, left);
    process_deletions(deletions, rhs.deletions, right);

    // === process updates ===
    process_updates(rhs.updates, *this, left);
    process_updates(updates, rhs, right);

    // === process insertions ===

    // deleted insertions first. it is important
    process_deleted_insertions(rhs.insertions, *this, root_state, left);
    process_deleted_insertions(insertions, rhs, root_state, right);

    // unique insertions
    process_unique_insertions(rhs.insertions, *this, left);
