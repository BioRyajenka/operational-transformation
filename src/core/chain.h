...
//        iterate_nodes([](const auto& n){ delete n; });
    ...
    void iterate(const std::function<void(const node_id_t &)> &consumer) const {
        auto cur = head;
        while (cur != nullptr) {
            consumer(cur->value.id);
            cur = cur->next;
        }
    }
